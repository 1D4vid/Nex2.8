return function(env)
    ...
    Library.CreateInput = function(self, targetPage, Text, Default, Callback)
        ...
        if newContainer then
            task.defer(function()
                for _, descendant in ipairs(newContainer:GetDescendants()) do
    ...
    for _, p in ipairs(Players:GetPlayers()) do
        if p.Character then addCharToCache(p.Character) end
        p.CharacterAdded:Connect(addCharToCache)
    ...
    local function setupBlackoutBypass(state)
        ...
        if state then
            ...
            if blackoutCharConn then blackoutCharConn:Disconnect() end
    ...
    local function rebuildPlayerNamesCache()
        table.clear(playerNamesCache)
        for _, p in ipairs(Players:GetPlayers()) do
            if p.Name ~= "" then playerNamesCache[p.Name] = true end
            if p.DisplayName and p.DisplayName ~= "" then
                playerNamesCache[p.DisplayName] = true
            end
        end
    end
    table.insert(playerNamesConnections, Players.PlayerAdded:Connect(rebuildPlayerNamesCache))
    table.insert(playerNamesConnections, Players.PlayerRemoving:Connect(rebuildPlayerNamesCache))
    rebuildPlayerNamesCache()
    ...
    local function limparTextoUI(element)
        ...
        local function verificar()
            ...
            local mudou = false
            for name in pairs(playerNamesCache) do
                if txt:find(name, 1, true) then
                    txt = txt:gsub(name, "")
                    mudou = true
                end
            end
    ...
    local function setupCamBlur(state)
        ...
        if state then
            ...
            if not motionBlur then
                motionBlur = camera:FindFirstChild("NexMotionBlur") or Instance.new("BlurEffect")
            ...
            camBlurConn = RunService.Heartbeat:Connect(function()
                ...
                local currentLook = currentCam.CFrame.LookVector
                local magnitude = (currentLook - lastVector).Magnitude
                motionBlur.Size = math.clamp(magnitude * 100, 0, 56)
                lastVector = currentLook
            end)
        else
    ...
    local activePlayers = {} 
    local clonedEnvironment = setmetatable({}, {__mode = "k"}) 
        ...
        if state then
            ...
            local function cleanPart(part)
                ...
                if not part.CanCollide then
                    ...
                    while current and current ~= workspace do
                        if characterCache[current] then
                            isCharPart = true
                            break
                        end
                        current = current.Parent
                    end
                    if isCharPart then return end

                    if name:find("leaf") or name:find("bush") or name:find("grass") or name:find("tree") or mat == Enum.Material.Grass or mat == Enum.Material.LeafyGrass or isGreen(part.Color) then
